...
- Added one-sided formula interfaces for the presence and positive components
  and a `random = ~ 1 | subject` interface. The earlier character-vector
  arguments remain temporarily available with deprecation warnings.
- Added non-adaptive Gauss--Hermite integration, multiple starting values,
