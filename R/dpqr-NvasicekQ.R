...
#' \eqn{\sigma} is a shape parameter. The fixed level is supplied through
#' the \code{quantile} argument. The functions
...
#' @param quantile Fixed quantile level \eqn{\tau\in(0,1)} used in the
#'   distribution functions and in the \code{NVASIQ()} GAMLSS family.
#'
#' @return
...
#' and \eqn{\sigma} is a shape parameter. The level supplied through
#' \code{quantile} is stored in the family definition and embedded as a
...
#'
#' where \eqn{0<x<1}, \eqn{0<\mu<1}, \eqn{0<\sigma<1}, and
#' \eqn{0<\tau<1}; \eqn{\mu} is the \eqn{\tau}-th quantile and
#' \eqn{\sigma} is the shape parameter.
#'
#' @examples
#' set.seed(123)
...
#' @rdname NVASIQ
#' @export
dNVASIQ <- function(x, mu, sigma, quantile = 0.50, log = FALSE) {
    ...
    cpp_dNVASIQ(x, mu, sigma, quantile, log)
}

##################################################
#' @rdname NVASIQ
#' @export
pNVASIQ <- function(q, mu, sigma, quantile = 0.50,
    ...
    cpp_pNVASIQ(q, mu, sigma, quantile, lower.tail, log.p)
}

##################################################
#' @rdname NVASIQ
#' @export
qNVASIQ <- function(p, mu, sigma, quantile = 0.50,
                    lower.tail = TRUE, log.p = FALSE) {
    ...
    .check_scalar_logical(lower.tail, "lower.tail")
    .check_scalar_logical(log.p, "log.p")
    cpp_qNVASIQ(p, mu, sigma, quantile, lower.tail, log.p)
}

##################################################
#' @rdname NVASIQ
#' @export
rNVASIQ <- function(n, mu, sigma, quantile = 0.50) {
    n <- .n_random(n)
    .check_unit_interval(mu, "mu")
    .check_unit_interval(sigma, "sigma")
    quantile <- .check_fixed_quantile(quantile)
    cpp_qNVASIQ(runif(n), mu, sigma, quantile, TRUE, FALSE)
}

##################################################
#' @rdname NVASIQ
#' @export
NVASIQ <- function(quantile = 0.50, mu.link = "logit",
                   sigma.link = "logit") {
    quantile <- .check_fixed_quantile(quantile, "NVASIQ")
