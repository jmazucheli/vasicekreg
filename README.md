...
exact boundary values. Dedicated two-part random-intercept functions are also
available for zero-augmented longitudinal responses.
...
Gauss--Hermite quadrature. The preferred interface uses one-sided formulas
...
subject--time pairs. A time trend is fitted only when the time variable is
included in `formula_bin` and/or `formula_cont`. Joint likelihood-ratio
tests require identical design matrices in the two components. The joint AIC
and BIC use the complete two-part likelihood. BIC uses the
number of subjects in its penalty, following the `PROC NLMIXED` convention.
Component-wise AIC values sum to the joint AIC. Component-wise BIC values
do not generally sum to the joint BIC because the positive component uses
only subjects with at least one positive response.
...
The names returned by `coef()` and the corresponding `vcov()` dimension
names use `gamma_` and `beta_`, including the intercepts.
